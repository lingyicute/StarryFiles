...
import org.lyi.android.files.compat.PreferenceManagerCompat
...
import org.lyi.android.files.file.asExternalStorageUriOrNull
...
import org.lyi.android.files.navigation.StandardDirectorySettings
import org.lyi.android.files.provider.archive.ArchiveFileSystem
import org.lyi.android.files.provider.common.ByteString
import org.lyi.android.files.provider.common.moveToByteString
import org.lyi.android.files.provider.content.ContentFileSystem
import org.lyi.android.files.provider.document.DocumentFileSystem
import org.lyi.android.files.provider.document.resolver.ExternalStorageProviderHacks
import org.lyi.android.files.provider.linux.LinuxFileSystem
import org.lyi.android.files.provider.root.RootStrategy
import org.lyi.android.files.provider.sftp.SftpFileSystem
import org.lyi.android.files.provider.smb.SmbFileSystem
import org.lyi.android.files.storage.DocumentTree
import org.lyi.android.files.storage.FileSystemRoot
import org.lyi.android.files.storage.PrimaryStorageVolume
import org.lyi.android.files.util.StableUriParceler
import org.lyi.android.files.util.asBase64
import org.lyi.android.files.util.readParcelable
import org.lyi.android.files.util.readParcelableListCompat
import org.lyi.android.files.util.toBase64
import org.lyi.android.files.util.toByteArray
import org.lyi.android.files.util.use
        ...
        Parcel.obtain().use { newParcel ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    oldParcel.readInt()
                    val className = oldParcel.readString()
                    if (className == "org.lyi.android.files.storage.SmbServer") {
                        newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                        newParcel.writeString("org.lyi.android.files.storage.SmbServer")
                        ...
                        newParcel.writeString(
                            "org.lyi.android.files.provider.smb.client.Authority"
                        )
                        ...
                        oldParcel.readString()
                        newParcel.writeString(
                            "org.lyi.android.files.provider.smb.client.Authentication"
                        )
...
private fun migratePath1_4_0(oldParcel: Parcel, newParcel: Parcel) {
    ...
    when (className) {
        ...
        "org.lyi.android.files.provider.sftp.SftpPath" ->
            newParcel.writeParcelable(oldParcel.readParcelable<SftpFileSystem>(), 0)
        "org.lyi.android.files.provider.smb.SmbPath" ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "org.lyi.android.files.storage.SftpServer" -> {
                            newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                            newParcel.writeString("org.lyi.android.files.storage.SftpServer")
                            val id = oldParcel.readLong()
                            newParcel.writeLong(id)
                            val customName = oldParcel.readString()
                            newParcel.writeString(customName)
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "org.lyi.android.files.storage.SftpServer" -> {
                            ...
                            val authenticationPasswordOrPrivateKey = oldParcel.readString()
                            newParcel.writeString(authenticationPasswordOrPrivateKey)
                            if (authenticationClassName == "org.lyi.android.files.provider.sftp"
                ...
                Parcel.obtain().use { oldParcel ->
                    ...
                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                        ...
                        repeat(size) {
                            ...
                            oldParcel.setDataPosition(oldPosition)
                            when (className) {
                                "org.lyi.android.files.storage.DocumentManagerShortcut" -> {
                                    newParcel.writeInt(oldParcel.readInt())
                                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                                        oldParcel.readString()
                                        newParcel.writeString(
                                            "org.lyi.android.files.storage" +
